 ...
 *  Descri��o: Arquivo principal contendo todas as rotinas para funcionamento do equipamento
 ...
 *     BOOSTXL-DRV8301
 *   Configura��o de Jumpers para DEBUG:
 *     S4 - ON
 *     S1 - ON-ON-ON/UP-UP-UP
 *   Configura��o de Jumpers para BOOT DA FLASH:
 *     S4 - ON
...
MATH_vec2 phasor;             //Auxiliar para obten��o do �ngulo do rotor
...
***************************************************************************/
// #define MIN_VALUE_HALL   _IQ(0.2)        //(Padr�o 10%)  Valor m�nimo percentual para sensibiliza��o de pressionamento dos sensores de efeito hall
// #define MAX_VALUE_HALL   _IQ(0.8)        //(Padr�o 1 - IQ_24) Valor m�ximo do fundo de escala do sensor hall
// #define SIZE_STEPS_kRPM  _IQ(0.5)          //(Padr�o 200RPM) Valor em kilo RPM de resolu��o de velocidade
// #define SENSOR_CW        hallSensor1     //(Padr�o H1) Define o sensor para o sentido de rota��o hor�rio
// #define SENSOR_CCW       hallSensor2     //(Padr�o H2) Define o sensor para o sentido de rota��o anti-hor�rio
// #define MAX_kRPM_VALUE   _IQ(29)       //(Padr�o 15.600RPM) Valor em kilo RPM de velocidade m�xima da Pistola
// #define ACCEL_IND_VALUE  _IQ(126)        //(Padr�o 126 kRPM/s) Valor em kilo RPM por segundo da acelera��o em acionamento individual dos sensores
...
// #define MAX_ACCEL_VALUE  _IQ(126)        //(Padr�o 126 kRPM/s)Maior valor real de acelera��o poss�vel de se aplicar ao motor, valor em kilo RPM por segundo da acelera��o em acionamento duplo dos sensores
// #define SIZE_STEPS_ACCEL _IQ(50)         //(Padr�o 30kRPM/s) Valor em kilo RPM por segundo de resolu��o de acelera��o do pressionamento duplo dos gatilhos
// //#define AXIS_FREE_ZERO_S               //(Padr�o comentada - PERIGO - Descomentar PEGA FOGO) Macro que define o estado do eixo em velocidade zero, Comentada-eixo travado, N�o comentada-eixo solto AVISO: Atualmente ao acionar esta op��o a fonte pode entrar em modo prote��o pelo solavanco do motor ao ligar o PWM
// //#define MASK_ADC_ENABLE                //(Padr�o comentada) Elimina a precis�o do sensor AD atrav�s de mascaramento

#define MIN_VALUE_HALL   _IQ(0.2)        //(Padrão 10%)  Valor mínimo percentual para sensibilização de pressionamento dos sensores de efeito hall
#define MAX_VALUE_HALL   _IQ(0.78)        //(Padrão 1 - IQ_24) Valor máximo do fundo de escala do sensor hall
#define SIZE_STEPS_kRPM  _IQ(0.3)          //(Padrão 200RPM) Valor em kilo RPM de resolução de velocidade
...
/***************************************************************************
 *  Fun��es definidas por: �nderson I. da Silva
***************************************************************************/
//As fun��es s�o declaradas como "static inline" para que o compilador
//otimize o processo de s�ntese, alocando o c�digo dentro da fun��o principal
...
static inline void initCalcs(HAL_Handle handle){
  ...
  HAL_enableTimer0Int(handle); //Habilita o TIMER0 utilizado no controle de rota��o do pressionamento duplos dos gatilhos
}
...
static inline void HAL_readHallData(HAL_Handle handle)
{
  ...
  HAL_Obj *obj = (HAL_Obj *)handle;
    ...
    STriggers.hallSensor1 = _IQ12toIQ((_iq)ADC_readResult(obj->adcHandle,ADC_ResultNumber_8))-MIN_VALUE_HALL;
    STriggers.hallSensor2 = _IQ12toIQ((_iq)ADC_readResult(obj->adcHandle,ADC_ResultNumber_9))-MIN_VALUE_HALL;
    ...
    //Filtro exponencial de p�lo �nico
...
static inline void setState()
{
  ...
  switch(mainState){
    ...
    break;
    case TriggerDouble:
 //      O princ�pio de funcionamento da comuta��o de velocidade � simples, utilizando o conceito de MRUV
 ...
 //      tido de rota��o. Para o c�lculo do tempo, considera-se sempre o tempo necess�rio  para  que o mo
...
/***************************************************************************
 *  Fim de fun��es definidas por: �nderson I. da Silva
***************************************************************************/
...
//    gMotorVars.Flag_enableRsRecalc = true;
    ...
    //Loop principal - Tecnicamente n�o deve sair deste la�o
    ...
    while(gMotorVars.Flag_enableSys)
      {
        ...
        if (delayReadADC == CYCLES_ADC_READ)
        {
          ...
          // Fun��o principal para operar a pistola
          setState();
          delayReadADC = 0;
        }
        else{
